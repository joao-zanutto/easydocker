...
func applyChromeStyles(s *Set) {
	...
	s.Chrome.TitleMeta = lipgloss.NewStyle().Foreground(TextSecondary).Background(SurfaceBG).Padding(0, 1)
	s.Chrome.Badge = lipgloss.NewStyle().Foreground(LightYellow).Background(MutedPurple).Padding(0, 1)
	...
	s.Tabs.ActiveTab = lipgloss.NewStyle().Bold(true).Foreground(Cyan).Underline(true).Padding(0, 1)
}
